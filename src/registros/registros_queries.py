...
def calcular_costos_insumos_mensuales(anio: int, mes: int):
    ...
    query = """
        ...
        WHERE
            YEAR(rc.fecha) = %s AND MONTH(rc.fecha) = %s
        GROUP BY
            rc.id_maquina
    """
...
def obtener_alquileres_mensuales():
    ...
    query = """
        SELECT
            ...
            m.id AS id_maquina, 
            m.costo_alquiler_mensual
        FROM
            maquinas m
        JOIN
            clientes c ON m.id_cliente = c.id
        WHERE
            m.id_cliente IS NOT NULL
        ORDER BY
            c.nombre
    """
...
def generar_reporte_facturacion_mensual(anio: int, mes: int):
    ...
    query = """
        SELECT
            ...
            COALESCE(SUM(maquinas.costo_alquiler_mensual), 0) AS total_alquiler,
            COALESCE(SUM(registro_consumo.cantidad_usada * insumos.precio_unitario), 0) AS total_insumos,
            COALESCE(SUM(maquinas.costo_alquiler_mensual), 0) + COALESCE(SUM(registro_consumo.cantidad_usada * insumos.precio_unitario), 0) AS total_a_cobrar
        FROM
            clientes
        LEFT JOIN
            maquinas ON clientes.id = maquinas.id_cliente
        LEFT JOIN
            registro_consumo ON maquinas.id = registro_consumo.id_maquina AND YEAR(registro_consumo.fecha) = %s AND MONTH(registro_consumo.fecha) = %s
        LEFT JOIN
            insumos ON registro_consumo.id_insumo = insumos.id
        GROUP BY
